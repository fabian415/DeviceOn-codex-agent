...
1. **編譯失敗**時,讀懂建置 log,判斷根因並修改 `package.json`(版號衝突、peer dependency 問題;必要時直接改程式碼配合新版 API)。
2. **編譯成功**時,讀懂 Trivy 報告,用 `npm ls` / `npm explain` 定位每個 fixable CVE 該改哪個套件版號,修改 `package.json`(直接依賴改版號,間接依賴用 `overrides`)。
...
| 檔案 | 用途 |
|---|---|
| `~/workspace/DeviceOn_Frontend/.env` | Azure DevOps 認證與設定,不進版控,不可印出內容 |
| `~/workspace/DeviceOn_Frontend/cve-loop.sh` | **這個流程主要呼叫的腳本**,見下方「指令合約」 |
| `~/workspace/DeviceOn_Frontend/<AZURE_DEVOPS_REPO>/` | 實際要修改 `package.json` 的地方;這個 git clone 跨多次 Codex 呼叫重複使用,不屬於下面的「session」 |
| `~/workspace/DeviceOn_Frontend/.cve-loop-session` | `cve-loop.sh` 自己維護,記錄目前是哪個 session,你不需要讀寫它 |
| `~/workspace/DeviceOn_Frontend/runs/<開始時間戳>/` | 一個 session 的所有機械產物都在這裡(見下方說明) |
...
| `~/workspace/DeviceOn_Frontend/generate-report.py` | `report` 指令背後呼叫的報表產生腳本,不需要、也不應該自己直接執行 |
...
`.env` 需要 `AZURE_DEVOPS_PAT`、`AZURE_DEVOPS_ORG`、`AZURE_DEVOPS_PROJECT`、`AZURE_DEVOPS_REPO`、`MAIN_BRANCH`、`AZURE_PIPELINE_NAME`。**PAT 絕對不要印出來或寫進任何會被 commit 的檔案**(`package.json`、commit message、這份文件本身都不行)。
...
工作目錄:修 `package.json` / 原始碼都在 `~/workspace/DeviceOn_Frontend/${AZURE_DEVOPS_REPO}` 下(`cve-loop.sh` 已經幫你 `git checkout` 到 `nightly-build` 分支)。

## 專案結構總覽

這是單一 Vite + Vue 3 專案(沒有像 monorepo 那樣的多個 Gradle project),`package.json` 在 repo 根目錄,主要目錄:

| 目錄 | 說明 |
|---|---|
| `src/` | Vue 元件、路由、store、API 呼叫等原始碼,修建置失敗需要改程式碼配合新版 API 時,主要改這裡 |
| `public/` | 靜態資源,原樣複製進 `dist/` |
| `config/` | Vite/build 相關的專案設定 |
| `package.json` | 所有第三方套件版本定義的地方(`dependencies`/`devDependencies`),**大部分 CVE 修補的最終落點就是改這裡的版號** |

**repo 沒有 `package-lock.json`**(在 `.gitignore` 內,CI 與本地都是先 `npm install` 現場產生),所以不能用 `npm ci`,一律用 `npm install`。CI(`azure-pipelines.yml`)用 Node.js `20.9.0`,本地環境要對齊(見 `ENV_SETUP.md`),否則 `npm install` 解出來的相依樹可能跟 CI 不一致。

CI 的 Trivy 掃描是對整個 repo 做 filesystem 掃描(`trivy fs --scanners vuln`),在 `npm install` 之後、`npm run build` 之前執行,所以掃到的是 `npm install` 依照 `package.json` 現場解析出來的相依樹(而不是打包後的 `dist/`)。
...
### 用 `npm ls` / `npm explain` 定位

```sh
cd ~/workspace/DeviceOn_Frontend/${AZURE_DEVOPS_REPO}
nvm use 20.9.0   # 若本機有裝多版本 Node,確保跟 CI 對齊
npm install --no-audit --no-fund   # 確保本地已經照 package.json 現況解過一次相依樹

npm ls <package> --all       # 列出這個套件在整棵相依樹裡出現的所有位置與版本
npm explain <package>        # 針對每個解析到的版本,說明是被誰(哪個上游套件)帶進來的
```

同一個套件可能在相依樹裡的不同路徑被解析成不同版本,`npm explain` 的輸出會列出每一條路徑,對照 Trivy 報告裡的 `InstalledVersion` 確認是哪一條路徑造成漏洞。

### 判斷修補位置

- **直接依賴**(`package.json` 的 `dependencies`/`devDependencies` 直接列出這個套件):直接把版號改成 `FixedVersion`(或更高、但仍相容的版本,見「版號政策」)。注意專案慣例用 `^x.y.z` 這種 caret range,改的時候要確認新版號仍落在合理範圍內,不要不小心把 range 改壞。
- **間接依賴**(`npm explain` 顯示是被別的套件帶進來的,例如某個 UI 套件內部依賴了舊版 `axios`):只改 `package.json` 裡「別的地方」通常沒有用,因為那個套件根本沒被直接列出。npm(8.3+,Node 20.9.0 內建的 npm 版本已支援)提供 `overrides` 欄位可以強制整棵樹使用指定版本:

  ```json
  {
    "overrides": {
      "<vulnerable-package>": "<fixed-version-or-higher>"
    }
  }
  ```

  如果同一個套件在不同上游依賴路徑需要不同版本(較少見),可以用巢狀寫法只覆蓋特定路徑:
  ...
  改完 `overrides` 之後一定要重新 `npm install` 一次,再用 `npm ls <package> --all` 確認解析出來的版本真的變了(npm 的相依解析在某些情況下可能仍選到別的版本,需要確認)。
...
- 一個檔案(`pipeline-logs/.../build-log.txt`):直接讀檔案內容,找 `npm ERR!`、`ERESOLVE`、`error during build`、`[vite]` 錯誤訊息附近的內容。

最常見的根因,依出現位置分兩類:

**`npm install` 階段失敗**

1. **`ERESOLVE` peer dependency 衝突**:npm 7+ 預設會嚴格檢查 peer dependency,升級某個套件版號後,可能跟另一個套件宣告的 peer dependency range 衝突。優先做法是把衝突雙方都升到彼此相容的版本(而不是加 `--legacy-peer-deps`/`--force` silently 蓋過去——那只是讓 npm 不檢查,不代表相依關係真的相容,執行期仍可能出問題)。
2. **版號不存在或打錯**:確認 npm registry 上真的有這個版本(`npm view <package> versions`)。
3. **`engines` 欄位限制**:少數套件的新版可能要求更高的 Node.js 版本,需要確認 CI 用的 Node `20.9.0` 是否滿足。

**`npm run build`(Vite 打包)階段失敗**

1. 先確認是不是版號打錯或 `overrides` 寫法有誤。
2. 若是 API 不相容(例如 UI 套件、Vite 外掛升級大版號後改了 export 名稱或參數),優先考慮改用該套件「能相容、且仍然修掉原本 CVE」的最低版本,而不是直接整個回退到升版前的版本(回退等於放棄這個 CVE 的修補)。
3. 若牽涉到 `overrides`,確認寫的版號跟實際解析結果一致(重新跑一次 `npm ls <package> --all` 確認)。
4. **若前面幾種都解不掉(新版真的移除/改了 API),直接去讀呼叫到該套件的專案原始碼**(`grep -rl "<package>"` 或依 import 名稱找 `src/` 底下的 `.js`/`.vue` 檔),照新版 API 改寫呼叫方式,讓專案跟新版套件相容。這是刻意允許的做法:目標是「CVE 修掉、還能編譯」,不是「不動程式碼」。改動範圍只到讓相容/能編譯為止,不要順便重構其他無關的地方。

   **但「能編譯」只是必要條件,不是目標本身**。任何時候都不能為了讓程式碼編譯過、或讓 pipeline 跑成功,就把原本會執行的邏輯換成丟例外、回傳 `null`/空結果、印警告後直接略過、或整段包 `try/catch` 吞掉錯誤、或在元件裡把某個功能整個拿掉——這種寫法建置會過、Trivy 也會顯示已修,但功能其實悄悄壞掉了,而且比明顯的建置失敗更難被發現。原本能用的功能,修完 CVE 之後也必須繼續能用。若新版 API 真的整個移除某個能力,先找同一套件裡功能對等的替代 API(通常只是改了名字或參數簽名,不是能力真的消失),或找官方遷移指南/CHANGELOG 確認正確用法;找到多個候選方案時,自己評估哪個最接近原本行為(功能覆蓋度、效能、與專案其他地方用法的一致性),選定後直接實作,**不要停下來等使用者決定**,也不要用丟例外或空實作繞過去。實作完成後回報使用者時,說明原本卡在哪、最後選了哪個替代方案、跟原本行為有什麼差異即可。
...
- **`npm ls`/`npm explain` 找不到指定套件**:確認先跑過 `npm install`(相依樹是安裝後才存在),或該套件其實已經不在目前的相依樹裡(可能上一輪已經處理掉了)。
- **改了版號或 `overrides` 但 `npm ls` 顯示版本沒變**:重新 `npm install` 過一次再檢查;若仍沒變,確認 `overrides` 的 key 拼字/巢狀結構是否對應到 `npm explain` 顯示的實際依賴路徑。
- **`BUILD_OK_NO_TRIVY_REPORT`**:目前腳本假設 Trivy 報告會出現在 artifact 裡路徑含 `Trivy-Report` 的 `*.json` 檔(對照 `azure-pipelines.yml` 裡 `publish: $(TrivyScan.jsonReport) artifact: Trivy-Report` 的設定)。如果 pipeline 改了發布方式會抓不到。這是流程停止點,不要自動繼續,只需診斷並回報:
...
- **`reason=PIPELINE_INFRA_ERROR` 的 `BUILD_FAILED`**:`log` 內容可能是觸發/等待 pipeline 流程的完整輸出(不是 npm/vite log),先確認是不是 PAT/pipeline 名稱/逾時這類基礎設施問題,不要誤當成程式碼問題去改 `package.json`。
